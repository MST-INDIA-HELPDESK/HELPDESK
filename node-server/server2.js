...
function generateTicket(data,lastInsertId) {
	var date=dateFormat(new Date(), "yyyy-mm-dd HH:MM:ss");
  	var data = {user_id: lastInsertId, first_name: req.body.fnm || '', last_name: req.body.lnm || '', email: req.body.email, subject: req.body.subject, message: req.body.message, created_date: date, updated_date: date, priority: 'Medium', status:'Open', topic_id: 1,attached_file:req.body.attached_file || null};	
...
app.post("/api/emailToTicket", function(req,res) {
	var email = JSON.stringify(req.body).replace(/[{}]/g,'')
	
	console.log('email :: ',req.body)
	let parser = new MailParser();   
	
	//simpleParser(email, (err, parsed) => { console.log(parsed) });
    ...
    */             
	...
	fs.createReadStream(email).pipe(mailparser);
	*/
...
app.post("/api/fbToTicket", function(req,res) {	
	console.log('POST - fbToTicket',req.body.entry[0].changes)
...
var id=req.body.id
var queryString ="SELECT tbl_tickets.id,tbl_tickets.user_id,CONCAT(tbl_tickets.first_name, ' ', tbl_tickets.last_name) AS user_name, tbl_tickets.email, subject, message, created_date, updated_date, priority,assignee, team,status,profile_pic as assignee_pic FROM tbl_tickets INNER JOIN tbl_user on tbl_user.user_id=tbl_tickets.user_id WHERE tbl_tickets.id="+id;
	connection.query(queryString, function(err, rows) {
		if(err) {
			console.log(err);
		}
...
var queryString="SELECT id,email,status,CONCAT(first_name, ' ', last_name) AS user_name, created_date, updated_date, email, subject, message, priority, assignee FROM tbl_tickets";
	...
	if(params.user_type_id == 2){
		if(params.date=="" && params.priority=="" && params.status=="" && params.assignee=="" && params.team==""){
...
app.post('/api/updateTicket',function(req, res) {
	var data = req.body.data;
	var updateData=[];
	console.log('change Assignee ',data.assignee)
	if(data.type=="status"){
...
app.post('/api/updateEmailTemp', function(req, res) {
    var data = {name:req.body.temp_name, status:req.body.temp_status, content:req.body.content};
    connection.query("UPDATE tbl_emailsettings_template SET ?", data, function(err1, rows1) {
	    if(err1) {
		    console.log("Error1 in MySql :: "+err1);
	    }
	    else {
